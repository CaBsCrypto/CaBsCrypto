/* ==========================================================================
   ...
   File: .agents/challenger_m1_2_r2/test_m1_js_r2.js
   Challenger 2: Gate 2 Re-evaluation Test Suite
   ========================================================================== */
...
console.log('===========================================================');
console.log(' Challenger 2 (Re-eval): M1 Empirical JS Runtime & Contract Test Suite');
console.log('===========================================================');
...
// --------------------------------------------------------------------------
// Test Suite 1: Syntax & Existence Verification
// --------------------------------------------------------------------------
...
// --------------------------------------------------------------------------
// Test Suite 2: DOM Sandbox & App Initialization
// --------------------------------------------------------------------------
...
function createDOMContext() {
  ...
  let setTransformCalls = [];
  let scaleCalls = [];
  
  const mockCanvas = new MockElement('bg-canvas', 'CANVAS');
  mockCanvas.getContext = () => ({
    scale: (sx, sy) => { scaleCalls.push({ sx, sy }); },
    setTransform: (a, b, c, d, e, f) => { setTransformCalls.push({ a, b, c, d, e, f }); },
    clearRect: () => {},
    beginPath: () => {},
    arc: () => {},
    fill: () => {},
  ...
  const sandbox = {
    ...
    scaleCalls: scaleCalls,
    activeAnimFrames: activeAnimFrames
  };
...
// --------------------------------------------------------------------------
// Test Suite 5: Canvas Transform Matrix & Visibility Stress Test
// --------------------------------------------------------------------------
console.log('\n--- Suite 5: Canvas Transform Matrix & Visibility Stress Test ---');

// 5.1 Transform Matrix Reset Check
assert(sandbox.setTransformCalls.length > 0, 'setTransform called on canvas setup');
const lastTransform = sandbox.setTransformCalls[sandbox.setTransformCalls.length - 1];
assert(lastTransform.a === 2 && lastTransform.d === 2, 'setTransform scaled cleanly by DPR (2)');

// 5.2 Visibilitychange Event Listener check
const visListeners = sandbox.docListeners['visibilitychange'] || [];
assert(visListeners.length > 0, 'visibilitychange event listener registered');

// Simulate hidden tab
sandbox.document.hidden = true;
visListeners.forEach(fn => fn());
assert(sandbox.activeAnimFrames.size === 0, 'Animation frames cancelled when tab hidden');

// Simulate tab becoming visible again
sandbox.document.hidden = false;
visListeners.forEach(fn => fn());
assert(sandbox.activeAnimFrames.size === 1, 'Animation loop restarted with exactly 1 active frame when tab visible');

// Simulate redundant visibilitychange calls while visible
visListeners.forEach(fn => fn());
visListeners.forEach(fn => fn());
assert(sandbox.activeAnimFrames.size === 1, 'Redundant visibilitychange calls do NOT duplicate render loops');
